...
> **Note:** `FORGEOS_API_KEY` is your ForgeOS API key. Get it from the [ForgeOS dashboard](https://app.synctek.io) after registering. The `FORGEOS_ENGINE_URL` and `FORGEOS_API_KEY` env vars are read directly by the MCP server (distinct from the `X-ForgeOS-API-Key` HTTP header used by direct REST calls).
