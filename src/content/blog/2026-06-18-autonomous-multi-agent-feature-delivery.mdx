...
The test suite was small but surgical. One test pinned a subtle boundary: a page labeled "07" is numerically 7 but isn't an exact string match for "7," so it must resolve through the numeric fallback, and an inclusive `<=` bound is critical, because a `<` would skip the equal page.
